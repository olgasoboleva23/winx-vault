...
for (const t of dv.pages('"Tasks"').file.tasks.where(t => t.completed && t.completion)) {
...
const habitDone = {};
for (const page of dv.pages('"Daily Notes"').where(p => p.date?.ts >= start.ts && p.date?.ts <= end.ts)) {
