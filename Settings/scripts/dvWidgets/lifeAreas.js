...
const config = eval(await app.vault.adapter.read("Settings/scripts/dvWidgets/config.js"));
...
for (const t of dv.pages('"Tasks"').file.tasks.where(t => t.completed && t.completion?.ts <= sel.ts)) {
...
for (const page of dv.pages('"Daily Notes"').where(p => p.date?.ts >= monthStart.ts && p.date?.ts <= monthEnd.ts)) {
